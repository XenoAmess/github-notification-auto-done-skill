...
To run every 10 minutes, add a cron entry:

```cron
*/10 * * * * cd /path/to/repo && /path/to/venv/bin/python -m github_notification_auto_done --auto-rebase >> /var/log/github_cleanup.log 2>&1
```
